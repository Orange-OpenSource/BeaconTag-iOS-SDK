...
if [ -n "$MULTIPLATFORM_BUILD_IN_PROGRESS" ]; then
  echo "MULTIPLATFORM_BUILD_IN_PROGRESS --> EXITING !"
  exit 0
fi
...
echo "Create directory ${UNIVERSAL_OUTPUTFOLDER}"
...
ditto "${PROJECT_DIR}/${PROJECT_NAME}.framework" "${HOME}/Desktop/${PROJECT_NAME}-${VERSION_STRING}/${PROJECT_NAME}.framework"
